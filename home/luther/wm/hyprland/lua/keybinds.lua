...
hl.bind(mainMod .. " + T", hl.dsp.exec_cmd(terminal), { description = "Apps: Launch terminal (" .. format_app_name(terminal) .. ")" })
hl.bind(mainMod .. " + Return", hl.dsp.exec_cmd(terminal), { description = "Apps: Launch terminal (" .. format_app_name(terminal) .. ")" })
...
-- Throw active window to workspace silently (Super + Alt + [1..9, 0])
for i = 1, max_workspaces do
    local key = i % 10
    -- Switch workspace
    hl.bind(mainMod .. " + " .. key, hl.dsp.focus({ workspace = i }), { description = "Workspaces: Switch to workspace " .. i })
    -- Throw active window to workspace silently (stay on current workspace)
...
hl.bind("XF86AudioRaiseVolume", hl.dsp.exec_cmd("pamixer -u -i 5"), { locked = true, repeating = true, description = "Media: Volume up (+5%)" })
hl.bind("XF86AudioLowerVolume", hl.dsp.exec_cmd("pamixer -u -d 5"), { locked = true, repeating = true, description = "Media: Volume down (-5%)" })
